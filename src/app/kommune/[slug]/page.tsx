    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <nav className="border-b border-white/10 bg-black/30 backdrop-blur-sm">
        <div className="mx-auto flex max-w-7xl items-center gap-1 px-4 py-3 text-sm sm:px-6 lg:px-8">
          ...
          <span className="mx-1.5 text-gray-600">›</span>
          <span className="text-white font-medium">{municipality.name}</span>
        </div>
      </nav>
      ...
      <header className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
        ...
        <p className="mt-3 max-w-2xl text-base leading-7 text-gray-300">
          ...
          {stats.totalCapacity.toLocaleString('da-DK')} registrerede pladser i kommunen.
        </p>

        {/* Stats */}
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <StatCard label="Adresser" value={groups.length} />
          <StatCard label="Registreringer" value={municipality.activeShelterCount} />
          <StatCard label="Kapacitet" value={stats.totalCapacity} />
          <StatCard label="Postområder" value={stats.postalAreaCount} />
        </div>
      </header>
