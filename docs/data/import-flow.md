...
5. Mark `source_scan_complete = true` only after a complete uncapped traversal has durably checkpointed its final source page.
6. Call `app_v2.publish_datafordeler_import_v3(...)` with both the raw BBR/DAR counters and the staged-row count.
7. In one database transaction, the publication function:
...
8. Delete staging rows after publication, rejection, or an intentionally capped run.
9. Run public read-model, parity, and production smoke checks.
...
Write and resume modes require `DATAFORDELER_API_KEY`, `SUPABASE_URL`, and the server-only `SUPABASE_SECRET_KEY`. Finalization requires only the two Supabase values because it reuses an already completed source scan. A privileged key must never use a `NEXT_PUBLIC_*` name.
