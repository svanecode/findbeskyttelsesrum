-- Fix location type - it's geometry, not JSONB. Cast it to JSONB for return
...
BEGIN
  ...
  WITH shelter_groups AS (
    SELECT
      ...
      s.postnummer,
      s.kommunekode,
      s.location::jsonb as location,
      COUNT(*)::BIGINT as shelter_count,
      SUM(s.shelter_capacity)::BIGINT as total_capacity,
